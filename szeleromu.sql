...
--
-- Gép: 127.0.0.1
-- Létrehozás ideje: 2018. Jún 14. 18:03
...
--
-- A nézet helyettes szerkezete `2. feladat megoldása`
-- (Lásd alább az aktuális nézetet)
--
CREATE TABLE `2. feladat megoldása` (
`nev` varchar(255)
,`kezdev` int(255)
);
...
--
-- A nézet helyettes szerkezete `3.feladat megoldása`
-- (Lásd alább az aktuális nézetet)
--
CREATE TABLE `3.feladat megoldása` (
`nev` varchar(255)
,`kezdev` int(255)
);
...
CREATE TABLE `torony` (
  ...
  `teljesitmeny` int(255) NOT NULL,
  `kezdev` int(255) NOT NULL,
  `helyszinid` int(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_hungarian_ci;
...
--
-- Nézet szerkezete `2. feladat megoldása`
--
DROP TABLE IF EXISTS `2. feladat megoldása`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `2. feladat megoldása`  AS  select distinct `helyszin`.`nev` AS `nev`,`torony`.`kezdev` AS `kezdev` from (`helyszin` join `torony`) where (`torony`.`kezdev` = 2009) order by `helyszin`.`nev` ;

-- --------------------------------------------------------

--
-- Nézet szerkezete `3.feladat megoldása`
--
DROP TABLE IF EXISTS `3.feladat megoldása`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `3.feladat megoldása`  AS  select distinct `helyszin`.`nev` AS `nev`,`torony`.`kezdev` AS `kezdev` from (`helyszin` join `torony`) order by `torony`.`kezdev` desc limit 1 ;
